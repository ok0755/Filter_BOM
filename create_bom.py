...
if __name__=='__main__':
    ...
    v=[]
    i=0
    enter=raw_input(u'Input model,like as:"UM","UK":>>>')
    ...
    for x in results:
        try:
            txtvalue=x.get()
            #print txtvalue[0].split('.')[0]
            lists=re.findall(re_,txtvalue[1])
            lists.sort()
            key=' \n'.join(lists)
            dic[txtvalue[0].split('.')[0]]=key
        except:
            pass
    writeExcel(dic,enter)
    print u'任务完成,{}{}.xlsx 已建立...\n Copyright by Zhoub'.format(os.getcwd(),enter)